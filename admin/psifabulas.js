...
const MACROCATEGORIAS = [
  ...
  {
    id:"macro_habitos", icone:"🌿", label:"Corpo, Saúde e Autocuidado",
...
const FERRAMENTAS_INTERATIVAS = [
  ...
  {key:"diagnostico-macroatividades", label:"Diagnóstico de Macroatividades x Desgastes"},
];
...
function FerramentaDiagnosticoMacro({ user }) {
    ...
    <div style={{padding:"0 4px"}}>
      <div style={{background:BG,borderRadius:14,padding:"20px 18px",marginBottom:18,borderLeft:`4px solid ${COR}`}}>
        ...
        <p style={{fontSize:14,color:"#374151",lineHeight:1.7,margin:0}}>
          Macroatividades são os <strong>grandes blocos de tempo</strong> que compõem seu dia — categorias amplas como "Trabalho", "Reuniões" ou "Cuidados Pessoais". Aqui você vai mapear quanto tempo e energia cada uma consome, e descobrir quais realmente merecem esse custo.
        </p>
      </div>
      <div style={{background:"white",border:"1px solid #e5e7eb",borderRadius:12,padding:"16px 18px",marginBottom:18}}>
        <div style={{fontWeight:600,fontSize:14,marginBottom:10,color:"#374151"}}>📋 Como funciona:</div>
        <div style={{fontSize:13,color:"#6b7280",lineHeight:1.8}}>
          <div>1️⃣ <strong>Cadastre</strong> suas principais atividades do dia</div>
          <div>2️⃣ <strong>Classifique</strong> tempo, esforço mental e importância</div>
          <div>3️⃣ <strong>Veja</strong> o diagnóstico automático e reflexões</div>
        </div>
      </div>
      <button onClick={()=>setEtapa("cadastro")} className="btn btn-purple" style={{width:"100%",padding:14,fontSize:15}}>
        🚀 Começar mapeamento
      </button>
    </div>
  );
  ...
  if (etapa==="cadastro") return (
    <div style={{padding:"0 4px"}}>
      ...
      <p style={{fontSize:13,color:"#6b7280",marginBottom:16}}>Quais são os grandes blocos que compõem seu dia? Pode editar os nomes sugeridos.</p>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:16}}>
        {atividades.map((a,i)=>(
          <div key={i} style={{display:"flex",alignItems:"center",gap:8,background:"white",border:"1px solid #e5e7eb",borderRadius:10,padding:"10px 12px"}}>
            <input value={a.nome} onChange={e=>updateAtiv(i,"nome",e.target.value)}
              style={{flex:1,border:"none",outline:"none",fontSize:14,fontFamily:"inherit",color:"#111827"}}/>
      ...
      <div style={{marginBottom:16}}>
        <div style={{fontSize:13,color:"#6b7280",marginBottom:8,fontWeight:600}}>➕ Sugestões rápidas:</div>
        <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:12}}>
          {["Tarefas Domésticas","Exercícios","Lazer / Descanso","Deslocamentos","Redes Sociais"].map(s=>(
            <button key={s} onClick={()=>addAtividade(s)}
              style={{background:"#f3f4f6",border:"1px solid #e5e7eb",borderRadius:20,padding:"6px 14px",fontSize:13,cursor:"pointer",fontFamily:"inherit",color:"#374151"}}>
              + {s}
            </button>
          ))}
        </div>
        <div style={{display:"flex",gap:8}}>
          ...
          <button onClick={()=>addAtividade(novaAtiv)} className="btn btn-purple" style={{padding:"10px 16px"}}>➕ Adicionar</button>
        </div>
      </div>

      <button onClick={()=>setEtapa("classificacao")} className="btn btn-purple" style={{width:"100%",padding:14,fontSize:15}}
  ...
  if (etapa==="classificacao") return (
    <div style={{padding:"0 4px"}}>
      ...
      <p style={{fontSize:13,color:"#6b7280",marginBottom:16}}>Para cada atividade, indique tempo gasto, esforço mental e importância.</p>

      <div style={{display:"flex",flexDirection:"column",gap:16}}>
        {atividades.map((a,i)=>(
          <div key={i} style={{background:"white",border:"1px solid #e5e7eb",borderRadius:12,padding:"14px 16px"}}>
            <div style={{fontWeight:600,fontSize:14,color:"#111827",marginBottom:12}}>📌 {a.nome}</div>

            {/* Tempo */}
            <div style={{marginBottom:12}}>
              <div style={{fontSize:12,fontWeight:600,color:"#6b7280",marginBottom:6}}>⏰ Tempo gasto (por dia):</div>
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                <input type="number" min="0" max="24" value={a.horas} onChange={e=>updateAtiv(i,"horas",e.target.value)}
                  placeholder="0" style={{width:60,border:"1px solid #e5e7eb",borderRadius:8,padding:"8px 10px",fontSize:14,fontFamily:"inherit",textAlign:"center"}}/>
            ...
            <div style={{marginBottom:12}}>
              <div style={{fontSize:12,fontWeight:600,color:"#6b7280",marginBottom:6}}>
                🧠 Esforço mental: <strong style={{color:"#111827"}}>{["","Muito baixo","Baixo","Médio","Alto","Muito alto"][a.esforco]}</strong>
              </div>
              <input type="range" min="1" max="5" value={a.esforco}
            ...
            <div>
              <div style={{fontSize:12,fontWeight:600,color:"#6b7280",marginBottom:6}}>
                ⭐ Importância real: <strong style={{color:"#111827"}}>{["","Irrelevante","Pouco importante","Moderada","Importante","Essencial"][a.importancia]}</strong>
              </div>
              <input type="range" min="1" max="5" value={a.importancia}
                onChange={e=>updateAtiv(i,"importancia",+e.target.value)}
                style={{width:"100%",accentColor:"#d97706"}}/>
              <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:"#9ca3af"}}>
                <span>Irrelevante</span><span>Essencial</span>
              </div>
            </div>
          </div>
        ))}
      ...
      <div style={{display:"flex",gap:8,marginTop:20}}>
        <button onClick={()=>setEtapa("cadastro")} className="btn btn-ghost" style={{flex:1,padding:12}}>← Voltar</button>
        <button onClick={()=>setEtapa("resumo")} className="btn btn-purple" style={{flex:2,padding:12,fontSize:15}}>Ver diagnóstico →</button>
      </div>
    </div>
  );
  ...
  return (
    <div style={{padding:"0 4px"}}>
      <div style={{fontWeight:700,fontSize:15,marginBottom:4}}>📊 Diagnóstico de Macroatividades</div>
      <p style={{fontSize:13,color:"#6b7280",marginBottom:16}}>Veja como seu tempo e energia estão distribuídos.</p>
          ...
          return (
            <div key={i} style={{background:qi.bg,border:`1.5px solid ${qi.cor}30`,borderRadius:12,padding:"12px 16px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                ...
                <span style={{fontSize:11,fontWeight:700,color:qi.cor,background:"white",borderRadius:20,padding:"3px 10px",border:`1px solid ${qi.cor}40`,whiteSpace:"nowrap"}}>
                  {qi.label}
                </span>
              </div>
              <div style={{fontSize:12,color:"#6b7280",marginTop:6,display:"flex",gap:16,flexWrap:"wrap"}}>
                <span>⏰ {Math.floor(min/60)}h {min%60}min</span>
                <span>🧠 Esforço: {["","Muito baixo","Baixo","Médio","Alto","Muito alto"][a.esforco]}</span>
                <span>⭐ Importância: {["","Irrelevante","Pouco","Moderada","Importante","Essencial"][a.importancia]}</span>
              </div>
      ...
      {criticas.length > 0 && (
        <div style={{background:"#fef2f2",border:"1.5px solid #dc2626",borderRadius:12,padding:"14px 16px",marginBottom:20}}>
          <div style={{fontWeight:700,fontSize:14,color:"#dc2626",marginBottom:8}}>⚠️ Atenção — Atividades Críticas</div>
          <p style={{fontSize:13,color:"#374151",marginBottom:12,lineHeight:1.6}}>
            {criticas.length === 1 ? "A atividade abaixo consome" : `As ${criticas.length} atividades abaixo consomem`} muito tempo/esforço mas têm baixa importância. Considere:
          </p>
          <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:12}}>
            {criticas.map((a,i)=>(
              <div key={i} style={{fontSize:13,fontWeight:600,color:"#dc2626"}}>• {a.nome}</div>
            ))}
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {[
              "🤝 Posso delegar ou pedir ajuda com isso?",
              "⏱️ Existe uma forma de fazer em menos tempo?",
              "❌ O que aconteceria se eu simplesmente parasse de fazer?",
              "🔄 Isso está alinhado com o que é realmente importante para mim agora?",
            ].map((q,i)=>(
              <div key={i} style={{background:"white",borderRadius:8,padding:"10px 12px",fontSize:13,color:"#374151",borderLeft:"3px solid #dc2626"}}>
                {q}
              </div>
            ))}
          </div>
      ...
      <div style={{background:"white",border:"1px solid #e5e7eb",borderRadius:12,padding:"14px 16px",marginBottom:20}}>
        <div style={{fontWeight:600,fontSize:14,marginBottom:10}}>📈 Resumo do Seu Dia</div>
        <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
          <div style={{background:"#f9fafb",borderRadius:10,padding:"10px 16px",textAlign:"center",flex:1}}>
            <div style={{fontSize:20,fontWeight:700,color:"#111827"}}>{Math.floor(totalMin/60)}h {totalMin%60}min</div>
          ...
          <div style={{background:"#fef2f2",borderRadius:10,padding:"10px 16px",textAlign:"center",flex:1}}>
            ...
            <div style={{fontSize:11,color:"#6b7280"}}>Atividades críticas</div>
          </div>
      ...
      {salvo ? (
        <div style={{background:"#f0fdf4",border:"1px solid #16a34a",borderRadius:12,padding:16,textAlign:"center"}}>
          ...
          <div style={{fontWeight:600,color:"#16a34a"}}>Diagnóstico salvo com sucesso!</div>
          <div style={{fontSize:13,color:"#6b7280",marginTop:4}}>Sua psicóloga poderá ver este resultado no painel.</div>
        </div>
      ) : (
        <div style={{display:"flex",gap:8}}>
          <button onClick={()=>setEtapa("classificacao")} className="btn btn-ghost" style={{flex:1,padding:12}}>← Revisar</button>
          <button onClick={salvar} disabled={salvando} className="btn btn-purple" style={{flex:2,padding:12,fontSize:15}}>
  ...
  function RadarSVG({valores}){
    ...
    return(
      <svg width="280" height="280" viewBox="0 0 280 280">
        ...
        {pontos.map((p,i)=><circle key={i} cx={p.x} cy={p.y} r="4" fill="#7B00C4"/>)}
        {labels}
      </svg>
    );
  }
