...
const GRUPOS_DIAGNOSTICOS = [
  {
    ...
    corBg: "#f5f3ff",
    hipoteses: [
      { id: "bipolar_mania",  label: "Transtorno Bipolar I/II — Mania/Hipomania" },
      { id: "bipolar_dep",    label: "Transtorno Bipolar — Episódio Depressivo" },
      { id: "ciclotimia",     label: "Ciclotimia" },
      { id: "borderline",     label: "TP Borderline (TPB)" },
      { id: "histrionico",    label: "TP Histriônico" },
      { id: "narcisista",     label: "TP Narcisista" },
    ],
  },
  {
    id: "g2",
    ...
    corBg: "#ede9fe",
    hipoteses: [
      { id: "tdah_desatento",  label: "TDAH — Desatento" },
      { id: "tdah_hiperativo", label: "TDAH — Hiperativo/Impulsivo" },
      { id: "tea",             label: "TEA — Espectro Autista" },
      { id: "tod",             label: "TOD — Transtorno Opositivo Desafiador" },
    ],
  },
  {
    id: "g3",
    ...
    corBg: "#eff6ff",
    hipoteses: [
      { id: "tdm",         label: "Depressão Unipolar (TDM)" },
      { id: "distimia",    label: "Distimia / Depressão Persistente" },
      { id: "tag",         label: "TAG — Ansiedade Generalizada" },
      { id: "panico",      label: "Transtorno de Pânico" },
      { id: "fobia_social",label: "Fobia Social / Ansiedade Social" },
      { id: "toc",         label: "TOC — Obsessivo-Compulsivo" },
  ...
  },
  {
    ...
    hipoteses: [
      ...
      { id: "tca",      label: "TCA — Compulsão Alimentar" },
      { id: "arfid",    label: "ARFID — Evitação/Restrição Alimentar" },
    ],
  },
  {
    id: "g5",
    ...
    corBg: "#fef3c7",
    hipoteses: [
      { id: "substancias", label: "Transtorno por Uso de Substâncias" },
      { id: "gaming",      label: "Gaming Disorder — Jogos Digitais" },
      { id: "gambling",    label: "Gambling Disorder — Apostas" },
      { id: "antissocial", label: "TP Antissocial (≥18 anos)" },
    ],
  },
  {
    id: "g6",
    ...
    corBg: "#fdf2f8",
    hipoteses: [
      { id: "sexual", label: "Disfunções e Saúde Sexual" },
    ],
  },
  {
    id: "g7",
    ...
    corBg: "#f0fdfa",
    hipoteses: [
      { id: "tept_complexo", label: "TEPT Complexo / Trauma Crônico" },
      { id: "dissociativo",  label: "Transtorno Dissociativo" },
      { id: "adaptacao",     label: "Transtorno de Adaptação" },
    ],
  },
];
...
function PainelGrupoDiagnostico({ paciente, grupo, onVoltar }) {
  ...
  function gerarLink() {
    if (selecionados.length === 0) { alert("Selecione pelo menos uma hipótese diagnóstica."); return; }
    setGerandoLink(true);
    ...
    db.collection("clinica_rastreamento_tokens").doc(token).set(config)
      .then(() => {
  ...
  // Calcular resultado de uma resposta (resultados já vêm calculados do Firestore)
  function calcularResultado(resp) {
    return (resp.resultados || []).map(r => {
      // TEA: cor já vem calculada com lógica de espectro
      // Outros: recalcular cor pelo nC vs minC
      const cor = r.cor || (r.nC >= r.minC ? "#dc2626" : r.nC >= Math.ceil((r.minC||3) * 0.6) ? "#d97706" : "#16a34a");
      return { ...r, cor };
    });
  }
  ...
  return (
    <div>
      {/* Cabeçalho */}
      <button onClick={onVoltar} style={{display:"flex",alignItems:"center",gap:6,background:"none",border:"none",color:"var(--purple)",fontWeight:600,fontSize:13,cursor:"pointer",marginBottom:20,padding:0}}>
        <Icon name="arrow-left" size={15}/> Voltar para Questionários
      </button>

      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}>
        <div style={{fontSize:26}}>{grupo.emoji}</div>
        <div>
          <div style={{fontWeight:700,fontSize:15,color:"var(--text-dark)"}}>{grupo.titulo}</div>
          <div style={{fontSize:12,color:"var(--text-muted)"}}>{grupo.descricao}</div>
        </div>
      </div>

      {/* Seleção de hipóteses */}
      <div style={{background:"#f9fafb",border:"1px solid #e5e7eb",borderRadius:12,padding:16,marginBottom:16,marginTop:16}}>
        <div style={{fontWeight:600,fontSize:12,color:"var(--text-dark)",marginBottom:10}}>
          🎯 Selecione as hipóteses diagnósticas a avaliar:
        </div>
        {grupo.hipoteses.map(h => (
          <label key={h.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 10px",borderRadius:8,cursor:"pointer",marginBottom:4,background:selecionados.includes(h.id)?"#f5f3ff":"white",border:"1.5px solid",borderColor:selecionados.includes(h.id)?grupo.cor:"#e5e7eb",transition:"all .15s"}}>
            <input
              type="checkbox"
              checked={selecionados.includes(h.id)}
              onChange={() => toggleHipotese(h.id)}
              style={{accentColor:grupo.cor,width:15,height:15}}
            />
            <span style={{fontSize:13,color:"var(--text-dark)",fontWeight:selecionados.includes(h.id)?600:400}}>{h.label}</span>
          </label>
        ))}

        <div style={{display:"flex",gap:8,marginTop:14,flexWrap:"wrap"}}>
          <button onClick={gerarLink} disabled={gerandoLink || selecionados.length===0}
            style={{display:"flex",alignItems:"center",gap:6,background:grupo.cor,color:"white",border:"none",borderRadius:9,padding:"9px 16px",fontSize:12,fontWeight:600,cursor:selecionados.length===0?"not-allowed":"pointer",opacity:selecionados.length===0?0.5:1}}>
            <Icon name="link" size={13}/> {gerandoLink ? "Gerando…" : "Gerar Link"}
          </button>

          {linkGerado && (<>
            <button onClick={copiarLink}
              style={{display:"flex",alignItems:"center",gap:6,background:grupo.corBg,color:grupo.cor,border:"1px solid",borderColor:grupo.cor,borderRadius:9,padding:"9px 16px",fontSize:12,fontWeight:600,cursor:"pointer"}}>
              <Icon name="copy" size={13}/> Copiar Link
            </button>
            <button onClick={enviarWhatsApp}
              style={{display:"flex",alignItems:"center",gap:6,background:"#dcfce7",color:"#15803d",border:"none",borderRadius:9,padding:"9px 16px",fontSize:12,fontWeight:600,cursor:"pointer"}}>
        ...
        {linkGerado && (
          <div style={{marginTop:10,background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:8,padding:"8px 12px",fontSize:11,color:"#166534",wordBreak:"break-all"}}>
      ...
      {/* Respostas recebidas */}
      <div style={{fontWeight:600,fontSize:13,color:"var(--text-dark)",marginBottom:12}}>
        📥 Respostas recebidas ({loadingRespostas ? "…" : respostas.length})
      </div>

      {loadingRespostas ? (
        <div style={{padding:20,textAlign:"center"}}><Spinner/></div>
      ) : respostas.length === 0 ? (
        <div style={{background:"#f9fafb",border:"1px dashed #d1d5db",borderRadius:12,padding:28,textAlign:"center",color:"var(--text-muted)",fontSize:13}}>
          Nenhuma resposta recebida ainda.<br/>Gere o link acima e envie ao paciente.
        </div>
      ) : (
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {respostas.map(resp => {
            ...
            return (
              <div key={resp.id} style={{border:"1px solid #e5e7eb",borderRadius:12,overflow:"hidden"}}>
                {/* Cabeçalho da resposta */}
                <div onClick={() => setRespSelecionada(isOpen ? null : resp.id)}
                  style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 16px",cursor:"pointer",background:isOpen?"#f5f3ff":"white"}}>
                  <div>
                    <div style={{fontWeight:700,fontSize:13,color:"var(--text-dark)"}}>
                      {resp.tipoRespondente === "paciente" ? "🧑 Próprio paciente" : `👨‍👩‍👧 ${resp.nomeRespondente || "Familiar"} · ${resp.parentesco || ""}`}
                    </div>
                    <div style={{fontSize:11,color:"var(--text-muted)",marginTop:2}}>
                      {resp.createdAt?.seconds ? new Date(resp.createdAt.seconds * 1000).toLocaleDateString("pt-BR") : "—"}
                      {" · "}{resp.modulos?.length || 0} módulos avaliados
                    </div>
                  </div>
                  <div style={{textAlign:"right"}}>
                    {comCriterio.length > 0 ? (
                      <div style={{fontSize:12,fontWeight:700,color:"#dc2626"}}>⚠ {comCriterio.length} critério(s) atingido(s)</div>
                    ) : (
                      <div style={{fontSize:12,fontWeight:700,color:"#16a34a"}}>✅ Abaixo dos limiares</div>
                    )}
                    <div style={{fontSize:11,color:"var(--text-muted)",marginTop:2}}><Icon name={isOpen?"chevron-up":"chevron-down"} size={13}/></div>
                  </div>
                </div>

                {/* Detalhe por transtorno */}
                {isOpen && (
                  <div style={{padding:"0 16px 16px"}}>
                    {resultados.map(r => (
                      <div key={r.mod} style={{marginBottom:8,padding:"10px 12px",background:"#f9fafb",borderRadius:10,border:"1px solid #e5e7eb"}}>
                        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8}}>
                          <div style={{fontSize:12,fontWeight:700,color:"var(--text-dark)"}}>{r.label}</div>
                          <div style={{fontSize:11,fontWeight:700,color:r.cor,whiteSpace:"nowrap"}}>{r.nC}/{r.total} C</div>
                        </div>
                        <div style={{fontSize:11,color:r.cor,marginTop:3}}>{r.status}</div>
                        {r.obs && <div style={{fontSize:10,color:"var(--text-muted)",marginTop:4,lineHeight:1.5}}>{r.obs}</div>}
                      </div>
                    ))}
                    {resp.obsFinais && (
                      <div style={{marginTop:8,padding:"10px 12px",background:"#fffbeb",border:"1px solid #fcd34d",borderRadius:10,fontSize:12,color:"#78350f"}}>
                        <strong>Observações:</strong> {resp.obsFinais}
                      </div>
                    )}
                  </div>
                )}
  ...
  return (
    <div>
      ...
      <div style={{fontSize:12,color:"var(--text-muted)",marginBottom:20}}>
        Selecione um grupo para escolher as hipóteses diagnósticas e gerar o link personalizado.
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
        ...
        <div style={{border:"1px solid var(--gray-200)",borderRadius:14,padding:18,background:"white",display:"flex",flexDirection:"column",gap:12}}>
          <div style={{display:"flex",alignItems:"flex-start",gap:12}}>
            ...
            <div style={{flex:1}}>
              <div style={{fontWeight:700,fontSize:13.5,color:"var(--text-dark)",marginBottom:3}}>Anamnese</div>
        ...
        {GRUPOS_DIAGNOSTICOS.map(g => (
          <div key={g.id} style={{border:"1px solid var(--gray-200)",borderRadius:14,padding:18,background:"white",display:"flex",flexDirection:"column",gap:12}}>
            <div style={{display:"flex",alignItems:"flex-start",gap:12}}>
              <div style={{fontSize:26,lineHeight:1}}>{g.emoji}</div>
              <div style={{flex:1}}>
                <div style={{fontWeight:700,fontSize:13.5,color:"var(--text-dark)",marginBottom:3}}>{g.titulo}</div>
                <div style={{fontSize:11.5,color:"var(--text-muted)",lineHeight:1.5}}>{g.descricao}</div>
                <div style={{marginTop:5,display:"flex",flexWrap:"wrap",gap:4}}>
                  {g.hipoteses.map(h => (
                    <span key={h.id} style={{fontSize:9.5,background:g.corBg,color:g.cor,borderRadius:4,padding:"2px 6px",fontWeight:600}}>
                      {h.label.split(" — ")[0]}
                    </span>
                  ))}
                </div>
            ...
            <button onClick={()=>setSub(g.id)}
              style={{display:"flex",alignItems:"center",gap:5,background:g.corBg,color:g.cor,border:"none",borderRadius:8,padding:"7px 14px",fontSize:12,fontWeight:600,cursor:"pointer",alignSelf:"flex-start"}}>
              <Icon name="clipboard-list" size={13}/> Selecionar e Gerar Link
            </button>
          </div>
        ))}
