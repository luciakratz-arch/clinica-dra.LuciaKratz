  ...
  function RadarSVG({valores, cor}){
    ...
    return(
      <svg width="280" height="280" viewBox="0 0 280 280">
        {grades}{eixos}
        <polygon points={pts} fill={fillColor} stroke={c} strokeWidth="2"/>
        {pontos.map((p,i)=>(
  ...
  return(
    <div>
      ...
      {historico.length>=2&&(()=>{
        ...
        const valsAtual=docToVals(atual);
        const valsAnterior=docToVals(anterior);
        return(
          <div style={{marginTop:20,padding:"14px 12px",background:"#faf5ff",borderRadius:12,border:"1px solid #e9d5ff"}}>
            <div style={{fontWeight:700,color:"var(--purple)",fontSize:13,marginBottom:14,textAlign:"center"}}>📊 Comparação — Antes × Agora</div>
            <div style={{display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap"}}>
              <div style={{textAlign:"center",flex:"1 1 140px",maxWidth:200}}>
                <div style={{fontSize:11,fontWeight:700,color:"#6b7280",marginBottom:6,background:"#f3f4f6",borderRadius:20,padding:"3px 10px",display:"inline-block"}}>📅 {anterior.data}</div>
                <RadarSVG valores={valsAnterior} cor="#a78bfa"/>
              </div>
              <div style={{textAlign:"center",flex:"1 1 140px",maxWidth:200}}>
                <div style={{fontSize:11,fontWeight:700,color:"var(--purple)",marginBottom:6,background:"#ede9fe",borderRadius:20,padding:"3px 10px",display:"inline-block"}}>📅 {atual.data} (atual)</div>
                <RadarSVG valores={valsAtual} cor="#7B00C4"/>
              </div>
            </div>
            {/* Variações por área */}
            <div style={{marginTop:14,display:"flex",flexDirection:"column",gap:4}}>
              {AREAS.map(a=>{
                const vA=valsAnterior[a.id]||0;
                const vB=valsAtual[a.id]||0;
                const diff=vB-vA;
                const cor=diff>0?"#059669":diff<0?"#dc2626":"#9ca3af";
                const icone=diff>0?"▲":diff<0?"▼":"—";
                return(
                  <div key={a.id} style={{display:"flex",justifyContent:"space-between",fontSize:12,padding:"3px 6px",borderRadius:6,background:"white"}}>
                    <span style={{color:"#374151",fontWeight:600}}>{a.label}</span>
                    <span style={{color:cor,fontWeight:700}}>{icone} {vA}→{vB}</span>
                  </div>
                );
              })}
            </div>
      ...
      {historico.length>0&&(
        <div style={{marginTop:16}}>
          <div style={{fontSize:12,fontWeight:600,color:"var(--purple)",marginBottom:8}}>📅 Histórico de sessões</div>
          {historico.map((h,i)=>(
            <div key={i} style={{fontSize:11,color:"#6b7280",padding:"7px 10px",background:i===0?"#f3e8ff":"#f9fafb",borderRadius:8,marginBottom:4,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <span style={{fontWeight:i===0?700:400,color:i===0?"var(--purple)":"#6b7280"}}>{h.data}{i===0?" (atual)":""}</span>
              <span style={{fontSize:10}}>{(h.areas||[]).map(a=>`${a.area[0]}${a.valor}`).join(" ")}</span>
            </div>
          ))}
        </div>
      )}
...
function RecursosPaciente({ user, setTab, abaInicial }) {
  ...
  const ferrAtivas = user.ferramentasAtivas || [];
  const fabulasGlobal = ferrAtivas.includes("fabulas") || ferrAtivas.includes("mod2");
  const mod2Ativo = !!(user.modulosConfig?.mod2?.ativo);
  const idsFabulasMod2 = new Set();
  if (user.modulosConfig?.mod2?.ferramentas) {
    Object.entries(user.modulosConfig.mod2.ferramentas).forEach(([fid, fd]) => {
      if (fd?.ativo) idsFabulasMod2.add(fid);
    });
  }

  const ferramentasVisiveis   = ferramentas.filter(r => ids.has(r.id) || ids.has(r.formularioKey));
  const fabulasVisiveis       = fabulas.filter(r => {
    if (ids.has(r.id) || ids.has(r.formularioKey)) return true;
    if (idsFabulasMod2.has(r.id)) return true;
    if ((fabulasGlobal || mod2Ativo) && idsFabulasMod2.size === 0) return true;
    return false;
  });
  const psicoeducacaoVisiveis = psicoeducacao.filter(r => ids.has(r.id) || ids.has(r.formularioKey));
...
function FerramentasAluno({ user }) {
  ...
  const MACROS = [
    ...
    {id:"macro_habitos",         icone:"🌱", label:"Corpo, Saúde e Autocuidado",           cor:"#16a34a", bg:"#dcfce7"},
  ...
  const LEGADO = {
    ...
    "corpo":"macro_habitos","alimentacao":"macro_habitos","casal":"macro_casais",
    "musicoterapia":"macro_musico","avaliacao":"macro_aval",
    "compulsao_sexual":"macro_compulsao","compulsao":"macro_compulsao",
    "macro_compulsao":"macro_compulsao","macro_corpo":"macro_habitos",
    "compulsao_ciclo":"macro_compulsao","compulsao_habitos":"macro_compulsao",
    "compulsao_emocional":"macro_compulsao","compulsao_vinculos":"macro_compulsao","compulsao_aval":"macro_compulsao",
    ...
    "breathing-478":"macro_habitos","muscle-relaxation":"macro_habitos",
    ...
    "polyvagal-ladder":"macro_habitos","grounding-5senses":"macro_habitos","body-mind-journal":"macro_habitos",
    "aterramento-5-sentidos":"macro_habitos","escada-polivagal":"macro_habitos","diario-corpo-mente":"macro_habitos",
    ...
    "roda-vida-integral":"macro_habitos","diagnostico-macroatividades":"macro_habitos",
    "empilhamento-habitos":"macro_habitos","ritual-noturno":"macro_habitos",
    "mapa-bateria":"macro_habitos","mural-habilidades":"macro_habitos","regra-5-minutos":"macro_habitos",
    "differentiation-map":"macro_casais","triangulation-map":"macro_casais","mapa-diferenciacao":"macro_casais","mapa-triangulacao":"macro_casais",
    "compassionate-parenting-journal":"macro_casais","financial-three-maps":"macro_casais","3-mapas-financeiros":"macro_casais",
    "intimacy-map":"macro_casais","mapa-intimidade":"macro_casais","diario-parentalidade":"macro_casais",
    "cnv-record":"macro_relacionamentos","limits-map":"macro_relacionamentos","registro-cnv":"macro_relacionamentos","mapa-limites":"macro_relacionamentos",
    "mental-load-inventory":"macro_relacionamentos","conflict-cycle-map":"macro_relacionamentos",
    "active-listening":"macro_relacionamentos","ciclo-conflito":"macro_relacionamentos",
    "escuta-ativa":"macro_relacionamentos","carga-mental":"macro_relacionamentos",
    "chain-analysis":"macro_ansiedade","analise-cadeia":"macro_ansiedade",
    "behavioral-activation":"macro_humor","ativacao-comportamental":"macro_humor",
    "tipp-sos":"macro_humor","kit-sos-tipp":"macro_humor",
    "strategic-pause":"macro_humor","pausa-estrategica":"macro_humor",
    "self-compassion-journal":"macro_humor","diario-autocompaixao":"macro_humor",
    "rastreamento-compulsao-sexual":"macro_compulsao",
    "rastreamento-bipolar":"macro_aval","rastreamento-sexual":"macro_aval",
    "rastreamento-alimentar":"macro_aval","rastreamento-neuro":"macro_aval",
    "rastreamento-dependencia":"macro_aval","rastreamento-jogos":"macro_aval",
    ...
    "macro_musico":"macro_musico","macro_aval":"macro_aval",
  };
  function getMacro(r){
  ...
  if (abrindoPsico) return (
    <div>
      <BarraVoltar texto={abrindoPsico.titulo||"Psicoeducação"}/>
      <PsicoeducacaoAberta item={abrindoPsico} user={user} onVoltar={fechar}/>
    </div>
  );
  if (abrindo) return (
    <div>
  ...
  return (
    <div style={{maxWidth:980,margin:"0 auto"}}>
      ...
      {/* Abas */}
      <div style={{display:"flex",gap:8,marginBottom:20,borderBottom:"1px solid var(--gray-200)",flexWrap:"wrap"}}>
        {[["ferramentas","🔧 Ferramentas",ferramentas.length],["fabulas","📖 Fábulas",fabulas.length],["psicoeducacao","🎓 Psicoeducação",psico.length]].map(([id,lbl,n])=>(
          <button key={id} onClick={()=>setAba(id)}
            style={{display:"flex",alignItems:"center",gap:6,padding:"10px 16px",border:"none",background:"none",
            ...
            {lbl} <span style={{fontSize:11,background:aba===id?"var(--purple)":"var(--gray-200)",color:aba===id?"white":"var(--gray-600)",borderRadius:20,padding:"1px 8px",fontWeight:700}}>{n}</span>
          </button>
        ))}
      </div>
...
// ─── MEUS LAUDOS ─────────────────────────────────────────
function MeusLaudos({ user }) {
  const [laudos, setLaudos] = React.useState([]);
  const [externos, setExternos] = React.useState([]);
  const [loading, setLoading] = React.useState(true);

  React.useEffect(()=>{
    const emailNorm = (user.email||"").trim().toLowerCase();
    const nomeNorm  = (user.nome||"").trim().toLowerCase();
    const u1 = db.collection("clinica_laudos")
      .where("pacienteId","==",user.id)
      .where("status","==","enviado")
      .onSnapshot(s=>{
        setLaudos(s.docs.map(d=>({id:d.id,...d.data(),origem:"clinica"})));
      },()=>{});
    db.collection("clinica_laudos_externos").get().then(s=>{
      const docs = s.docs.map(d=>({id:d.id,...d.data(),origem:"externo"})).filter(d=>{
        const dEmail = (d.pacienteEmail||d.email||"").trim().toLowerCase();
        const dNome  = (d.pacienteNome||d.nome||"").trim().toLowerCase();
        return (emailNorm && dEmail === emailNorm) || (nomeNorm && dNome === nomeNorm);
      });
      setExternos(docs);
      setLoading(false);
    }).catch(()=>setLoading(false));
  ...
  const todos = [...laudos, ...externos].sort((a,b)=>{
    const dA = a.dataEmissao?.seconds||a.createdAt?.seconds||0;
    const dB = b.dataEmissao?.seconds||b.createdAt?.seconds||0;
    return dB - dA;
  });

  if(loading) return <div style={{textAlign:"center",padding:60}}><Icon name="loader" size={32}/></div>;

  return (
    <div style={{maxWidth:680,margin:"0 auto",padding:"0 4px"}}>
      <div style={{marginBottom:24}}>
        <div style={{fontSize:22,fontFamily:"var(--font-display)",fontWeight:700,color:"var(--purple)",marginBottom:4}}>
          📄 Meus Laudos
        </div>
        <div style={{fontSize:14,color:"var(--text-muted)"}}>
          Documentos e relatórios emitidos pela sua psicóloga.
        </div>
      </div>
      {todos.length===0?(
        <div style={{textAlign:"center",padding:60,color:"var(--text-muted)",background:"var(--purple-soft)",borderRadius:16}}>
          <Icon name="file-text" size={48}/>
          <div style={{marginTop:12,fontWeight:600,fontSize:15}}>Nenhum laudo disponível ainda</div>
          <div style={{fontSize:13,marginTop:6}}>Quando a Dra. Lucia emitir um documento para você, ele aparecerá aqui.</div>
        </div>
      ):(
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          {todos.map(l=>{
            const url = l.arquivoUrl || l.linkDrive || l.urlPdf || l.url || "";
            const tipo = l.tipoDocumento || l.tipo || "Documento";
            const data = l.dataEmissao?.seconds
              ? new Date(l.dataEmissao.seconds*1000).toLocaleDateString("pt-BR")
              : l.createdAt?.seconds
                ? new Date(l.createdAt.seconds*1000).toLocaleDateString("pt-BR")
                : l.enviadoEm ? new Date(l.enviadoEm).toLocaleDateString("pt-BR") : "—";
            const badge = l.origem==="externo"
              ? {label:"9&Self",bg:"#ede0fa",cor:"#7B00C4"}
              : {label:"Clínica",bg:"#d1fae5",cor:"#065f46"};
            return (
              <div key={l.id} style={{background:"white",borderRadius:14,padding:"18px 20px",boxShadow:"0 1px 6px rgba(0,0,0,0.07)",display:"flex",alignItems:"center",gap:16}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",marginBottom:3}}>
                    ...
                    <span style={{background:badge.bg,color:badge.cor,borderRadius:20,padding:"2px 10px",fontSize:11,fontWeight:600}}>{badge.label}</span>
                  </div>
                  <div style={{fontSize:12,color:"var(--text-muted)"}}>📅 {data}</div>
                  {l.observacoes&&<div style={{fontSize:12,color:"var(--text-muted)",marginTop:2,fontStyle:"italic"}}>{l.observacoes}</div>}
                </div>
                {url?(
                  <a href={url} target="_blank" rel="noreferrer"
                    style={{background:"var(--purple)",color:"white",borderRadius:10,padding:"10px 16px",fontSize:13,fontWeight:600,textDecoration:"none",display:"flex",alignItems:"center",gap:6,flexShrink:0}}>
                    <Icon name="external-link" size={14}/> Ver PDF
                  </a>
                ):(
                  <span style={{fontSize:12,color:"var(--text-muted)",flexShrink:0}}>Link indisponível</span>
                )}
              </div>
            );
          })}
...
// ─── VITRINE DE PRODUTOS ──────────────────────────────────
function VitrineProducts({ user }) {
  const [produtos, setProdutos] = React.useState([]);
  const [loading, setLoading] = React.useState(true);

  React.useEffect(()=>{
    db.collection("produtos_vitrine").get().then(s=>{
      const docs = s.docs.map(d=>({id:d.id,...d.data()})).filter(d=>d.ativo===true);
  ...
  return (
    <div style={{marginTop:28}}>
      <div style={{fontSize:16,fontWeight:700,color:"var(--purple)",marginBottom:14}}>
        🛍️ Recursos & Ferramentas
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:14}}>
        {produtos.map(p=>(
          <div key={p.id} style={{background:"white",borderRadius:16,overflow:"hidden",boxShadow:"0 2px 10px rgba(123,0,196,0.08)",border:"1px solid var(--purple-soft)"}}>
            {p.imagemUrl&&(
              <img src={p.imagemUrl} alt={p.titulo} style={{width:"100%",maxHeight:180,objectFit:"cover",display:"block"}}/>
            )}
            <div style={{padding:"16px 18px"}}>
              <div style={{fontWeight:700,fontSize:16,marginBottom:6}}>{p.titulo}</div>
              {p.descricao&&<div style={{fontSize:13,color:"var(--text-muted)",marginBottom:14,lineHeight:1.6}}>{p.descricao}</div>}
              {p.linkVendas&&(
                <a href={p.linkVendas} target="_blank" rel="noreferrer"
                  style={{display:"inline-block",background:"var(--purple)",color:"white",borderRadius:10,padding:"10px 20px",fontSize:14,fontWeight:600,textDecoration:"none"}}>
                  {p.textoBotao||"Saiba mais"}
                </a>
              )}
            </div>
          </div>
...
function AtivInventario({ user, casalId, onVoltar }) {
  ...
  React.useEffect(()=>{
    ...
    // Buscar resposta do parceiro — chaves invertidas: o doc dele tem pacienteId=ele e casalId=eu
    db.collection("clinica_casais_respostas")
...
function AtivRodaVida({ user, casalId, onVoltar }) {
  ...
  React.useEffect(()=>{
    ...
    db.collection("clinica_casais_respostas")
      .where("casalId","==",user.id).where("pacienteId","==",casalId)
      .where("atividadeId","==","roda-vida-relacionamento")
  ...
  return (
    <div>
      ...
      <div className="main-content">
        ...
        {!eCasal&&tab==="meus-laudos"   &&<MeusLaudos user={user}/>}
