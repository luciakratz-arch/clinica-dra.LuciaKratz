  ...
  React.useEffect(()=>{
    ...
    db.collection("clinica_emergencia")
      ...
      .onSnapshot(s=>{
        ...
        const ativos = s.docs.map(d=>d.data()).filter(d=>d.fim&&d.fim>=agora);
        ativos.sort((a,b)=>b.fim>a.fim?1:-1);
        setAtivo(ativos.length>0?ativos[0]:null);
      },()=>{});
...
function AtivInventario({ user, casalId, onVoltar }) {
  ...
  React.useEffect(()=>{
    ...
    // Buscar resposta do parceiro — tenta por casalId e fallback por pacienteId
    db.collection("clinica_casais_respostas")
      .where("casalId","==",casalId)
      .where("atividadeId","==","inventario-bem-estar")
      .onSnapshot(s=>{
        ...
        } else if(casalId && casalId !== user.id){
          // Fallback: parceiro pode ter salvado com um casalId diferente
          db.collection("clinica_casais_respostas")
            .where("pacienteId","==",casalId)
            .where("atividadeId","==","inventario-bem-estar")
            .get().then(s2=>{
              const doc = s2.docs[0];
              if(doc) setRespParceiro(doc.data().respostas||{});
            }).catch(()=>{});
        }
      },()=>{});
  },[casalId, user.id]);
